...
- Refined the desktop theme around the new brand, added a local-processing indicator, and made disabled export controls visually unambiguous.
- Added verified product screenshots and an actual Moderate-preset comparison from a CPU run. Rebuilt the README around the real workflow, privacy model, downloads, and CLI.
- Fixed CPU warping on frame and ROI dimensions that are not divisible by the internal grid scale. A regression test now covers the failure.
...
- Added parameter clamping (0 to 100, 0 to 1024 for tile size) to CLI and manifest overrides to prevent out-of-range values causing visual corruption.
...
- **Remini / Topaz Photo AI:** upscale and face-enhance pipelines. Borrow their superres stage as
...
- **Preset marketplace:** signed JSON presets with thumbnails and community upload.
...
- **RetouchML** ([source](https://github.com/ju-leon/RetouchML)). StyleGAN2 latent-space beautification detects and normalizes the face, then moves it toward a learned attractiveness target.
- **photo-enhancer (nuwandda)** ([source](https://github.com/nuwandda/photo-enhancer)). It wraps GFPGAN for faces and Real-ESRGAN for backgrounds in one pipeline.
- **FaceEnhancementAndMakeup** ([source](https://github.com/ZainabZaman/FaceEnhancementAndMakeup)). DLIB landmarks drive makeup while CodeFormer handles restoration.
- **facefusion** ([source](https://github.com/facefusion/facefusion)). Its model zoo and pipeline graph are useful references for a larger manipulation platform.
- **Awesome Face Restoration** ([source](https://github.com/sczhou/Awesome-Face-Restoration)). This curated index tracks face-restoration papers and weights.
- **GFPGAN** ([source](https://github.com/TencentARC/GFPGAN)). It remains a common baseline for smoothing while retaining detail.
- **CodeFormer** ([source](https://github.com/sczhou/CodeFormer)). It performs well on blurry or low-resolution input and exposes a useful fidelity tradeoff.

### Features to Borrow
- Latent-space slimming slider from `RetouchML`: move along the StyleGAN2 "face width" direction instead of mesh warping. This preserves identity better than liquify.
- Two-stage face and background pipeline from `nuwandda/photo-enhancer`: segment the face, enhance it separately, then alpha-composite it back to avoid background smoothing artifacts.
- DLIB 68-point landmark gating from `FaceEnhancementAndMakeup`: run reshaping only inside the face polygon and skip ears or hair.
- Model-zoo selector like `facefusion`: let users pick between GFPGAN, CodeFormer, or RestoreFormer at runtime.
- CodeFormer's "fidelity weight" slider: let the user choose between strict identity preservation and stronger restoration.
...
- [ ] P0: Repair reproducible local environment setup
  ...
  Evidence: `.venv\Scripts\python.exe` failure, project working notes, and `requirements.txt`.
  Touches: README setup section, `requirements.txt`, optional setup script, and project working notes.
